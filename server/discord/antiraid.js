...
async function trigger(botId, guild, { count, window, action, unlockMin, byTag }) {
  ...
  let locked = 0;
  if (act === 'lockdown') {
    const r = await lockdown.on(botId, guild, byTag || '🛡️ Nexora — anti-raid automatique');
    locked = r.channels || 0;
  }
...
async function sweep(botId, entry, now = new Date()) {
  try {
    ...
    for (const key of allKeys) {
      ...
      await lockdown.off(botId, guild, '🛡️ Nexora — fin du verrouillage automatique');
...
async function unlockNow(botId, guild) {
  await lockdown.off(botId, guild, '🛡️ Nexora — réouverture manuelle');
