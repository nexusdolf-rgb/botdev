...
(async () => {
  ...
  check('carte textes du panneau à un bouton', chunk.includes('✏️ Textes du panneau à un bouton'));
  check('carte textes du panneau avec menu', chunk.includes('✏️ Textes du panneau avec menu'));
  ...
  check('panneau personnalisé : placeholder par défaut, plus de signature (v312)', jAdvDef.includes('🗂️ Choisissez un type de ticket…') && !jAdvDef.includes('Hoxera · Support privé'));

  console.log('— 9. Bump v297 —');
  const index = racine('public/index.html');
  check('index.html : ?v=328 référencé 7 fois', (index.match(/\?v=328/g) || []).length === 7, String((index.match(/\?v=328/g) || []).length));
  check('sw.js : cache « botdev-v328 »', racine('public/sw.js').includes("const CACHE = 'botdev-v328';"));
