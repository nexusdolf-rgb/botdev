...
async function main() {
  ...
  console.log('\n9) Version épinglée v238');
  {
    ...
    check('index.html : ?v=238 référencé 7 fois', (index.match(/\?v=238/g) || []).length === 7,
      `trouvé ${(index.match(/\?v=238/g) || []).length}`);
    check("sw.js : cache 'botdev-v238'", sw.includes("const CACHE = 'botdev-v238';"));
