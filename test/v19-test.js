...
(async () => {
  ...
  await panels.dispatchPanels(1, wizard('bpw-modal:1:OWNER1', { isModalSubmit: () => true, fields: { getTextInputValue: () => 'Hoxera du CHEAT' } }));
  assert(lastReply.embeds && lastReply.embeds[0].data.title.includes('Étape 1/5'), 'étape 1 attendue');
  assert(lastReply.embeds[0].data.fields[0].value.includes('Hoxera du CHEAT'), 'nom dans le récap');
  ...
  assert(p.name === 'Hoxera du CHEAT', 'nom : ' + p.name);
