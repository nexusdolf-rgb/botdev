...
(async () => {
  ...
  check('index.html : ?v=256 référencé 7 fois', versions.length === 7 && versions.every((v) => v === '?v=256'),
    `${versions.length} refs : ${[...new Set(versions)].join(',')}`);
  check('sw.js : cache « botdev-v256 »', swSource.includes("const CACHE = 'botdev-v256';"));
  check('index.html et sw.js portent la même version',
    swSource.includes("botdev-v256") && versions.every((v) => v === '?v=256'));
