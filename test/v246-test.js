...
(async () => {
  ...
  check('index.html : ?v=254 référencé 7 fois', versions.length === 7 && versions.every((v) => v === '?v=254'),
    `${versions.length} refs : ${[...new Set(versions)].join(',')}`);
  check('sw.js : cache « botdev-v254 »', swSource.includes("const CACHE = 'botdev-v254';"));
  check('index.html et sw.js portent la même version',
    swSource.includes("botdev-v254") && versions.every((v) => v === '?v=254'));
