...
(async () => {
  ...
  check('index.html : ?v=276 référencé 7 fois',
    versions.length === 7 && versions.every((v) => v === '?v=276'),
    `${versions.length} refs : ${[...new Set(versions)].join(',')}`);
  check('sw.js : cache « botdev-v276 »', swSource.includes("const CACHE = 'botdev-v276';"));
  check('index.html et sw.js portent la même version',
    swSource.includes('botdev-v276') && versions.every((v) => v === '?v=276'));
