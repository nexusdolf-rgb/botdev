...
(async () => {
  ...
  check('index.html : ?v=325 référencé 7 fois',
    versions.length === 7 && versions.every((v) => v === '?v=325'),
    `${versions.length} refs : ${[...new Set(versions)].join(',')}`);
  check('sw.js : cache « botdev-v325 »', swSource.includes("const CACHE = 'botdev-v325';"));
  check('index.html et sw.js portent la même version',
    swSource.includes('botdev-v325') && versions.every((v) => v === '?v=325'));
