...
w.fetch = async (url, opts) => {
  ...
  if (path.endsWith('/api/nexora')) return resp({ configured: true, bot: { id: 1, name: 'Nexora', prefix: '!', online: true, invite_url: 'https://x', status_text: '', avatar_url: '', bot_username: 'Nexora#1', guilds: [] } });
...
setTimeout(async () => {
  try {
    ...
    if (ok) { console.log('✅ DASHBOARD NEXORA RENDU PAR LE ROUTEUR — 100% fonctionnel'); process.exit(0); }
    process.exit(1);
