...
const dom = new JSDOM('<!DOCTYPE html><html><body><div id="app"></div><div id="toasts"></div><div id="modal-root"></div></body></html>', {
  url: 'http://localhost:3000/#/bots/1', runScripts: 'outside-only', pretendToBeVisual: true,
});
const w = dom.window;
...
w.fetch = async (url, opts) => {
  ...
  if (path.endsWith('/api/auth/me')) return resp({ user: { id: 1, email: 'a@b.fr', discord_id: 'D1', discord_username: 'a' } });
  if (path.endsWith('/api/bots/1')) return resp({ bot: { id: 1, name: 'Nexora', prefix: '!', online: true, invite_url: 'https://x', status_text: '', avatar_url: '', bot_username: 'Nexora#1', guilds: [] } });
...
setTimeout(async () => {
  try {
    await new Promise((r) => setTimeout(r, 800)); // laisse le routeur + fetchs finir
    ...
    const ok = results.modules >= 15 && results.header.includes('Vue') && results.guildPick === 'G1' && !results.errorShown;
    if (ok) { console.log('✅ DASHBOARD V2 RENDU PAR LE ROUTEUR — 100% fonctionnel'); process.exit(0); }
    process.exit(1);
