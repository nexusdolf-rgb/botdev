...
async function run() {
  ...
  const botId = store.bots.create({ user_id: 1, name: 'Hoxera', token: 'TOKEN_SECRET', client_id: '1', prefix: '!' });
  ...
  assert(bot && bot.name === 'Hoxera' && bot.token === 'TOKEN_SECRET', 'bot restauré avec token');
