...
(async () => {
  ...
  // --------------------------------------------------------------------------
  {
    ...
    check('index.html : ?v=303 référencé 7 fois', (html.match(/\?v=303/g) || []).length === 7,
      String((html.match(/\?v=303/g) || []).length));
    check('index.html : plus aucun ?v=242', !html.includes('?v=242'));
    check('sw.js : cache « botdev-v303 »', racine('public/sw.js').includes("'botdev-v303'"));
  }
