...
(async () => {
  ...
  // --------------------------------------------------------------------------
  {
    ...
    check('index.html : ?v=302 référencé 7 fois', (html.match(/\?v=302/g) || []).length === 7,
      String((html.match(/\?v=302/g) || []).length));
    check('index.html : plus aucun ?v=242', !html.includes('?v=242'));
    check('sw.js : cache « botdev-v302 »', racine('public/sw.js').includes("'botdev-v302'"));
  }
