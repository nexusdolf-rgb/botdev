...
(async () => {
  ...
  const globalModules = await fetchJson(`/bots/${targetBotId}/modules`, { headers: { Cookie: adminCookie } });
  const globalCommands = await fetchJson(`/bots/${targetBotId}/commands`, { headers: { Cookie: adminCookie } });
  assert.strictEqual(globalModules.status, 200, 'fondateur peut voir les modules du bot');
  assert.strictEqual(globalCommands.status, 200, 'fondateur peut voir les commandes du bot');
