...
(async () => {
  ...
  assert(asideAdmin.textContent.includes('Espace administrateur'), 'le fondateur voit le bouton admin');
  w.App.state.user = { id: 2, is_admin: false };
  const asideUser = w.document.createElement('aside');
  w.Dashboard.renderSide(asideUser);
  assert(!asideUser.textContent.includes('Espace administrateur'), 'un utilisateur normal ne voit pas le bouton admin');
