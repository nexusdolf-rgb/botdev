...
const DATA_DIR = path.join(os.tmpdir(), `botdev-v207-${Date.now()}`);
...
(async () => {
  ...
  check('index.html : version v193 référencée 7 fois', (indexHtml.match(/\?v=207/g) || []).length === 7);
  check('index.html : plus aucune référence v192', !indexHtml.includes('?v=192'));
  check('sw.js : cache v193', swSource.includes("const CACHE = 'botdev-v207';"));
