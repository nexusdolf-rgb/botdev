...
(async () => {
  ...
  }
  {
    ...
    check('index.html : ?v=319 référencé 7 fois', (html.match(/\?v=319/g) || []).length === 7,
      String((html.match(/\?v=319/g) || []).length));
    check('index.html : plus aucun ?v=240', !html.includes('?v=240'));
    check('sw.js : cache « botdev-v241 »', racine('public/sw.js').includes("'botdev-v319'"));
  }
