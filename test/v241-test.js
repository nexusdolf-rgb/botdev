...
const P = i18n.panelTexts('fr');
{
  ...
  // v306 — la signature « Hoxera · {serveur} » est retirée du pied (demande
...
(async () => {
  ...
  }
  {
    ...
    check('index.html : ?v=323 référencé 7 fois', (html.match(/\?v=323/g) || []).length === 7,
      String((html.match(/\?v=323/g) || []).length));
    check('index.html : plus aucun ?v=240', !html.includes('?v=240'));
    check('sw.js : cache « botdev-v241 »', racine('public/sw.js').includes("'botdev-v323'"));
  }
