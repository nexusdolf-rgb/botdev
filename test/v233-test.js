...
const DATA_DIR = path.join(os.tmpdir(), `botdev-v233-${Date.now()}`);
...
check('index.html : 7 références ?v=233', (read('public/index.html').match(/\?v=233/g) || []).length === 7);
check('index.html : plus aucune référence ?v=232', !read('public/index.html').includes('?v=232'));
check('sw.js : cache botdev-v233', read('public/sw.js').includes("const CACHE = 'botdev-v233';"));
