...
// v312 — plus de pied : 2 paragraphes (prix + message) + 1 bloc de compteurs = 3 blocs → 2 séparateurs.
check('2 séparateurs natifs (entre blocs, plus de pied v312)', nDiv(pDef) === 2);
const p2par = giveaway.buildPanel(G, { message: 'Réagissez avec 🎉 pour participer !\n\nSeuls les membres du serveur sont éligibles.' });
check('message utilisateur à 2 paragraphes → 3 séparateurs (plus de pied v312)', nDiv(p2par) === 3);
...
check('3 séparateurs natifs sur le panneau final (plus de pied v312)', nDiv(pEnd) === 3);
...
check('plus de pied « Hoxera · Événements · ID » (v312)', !txts(pEv).some((t) => t.includes('Hoxera ·') || t.includes(`ID ${ev.id}`) && t.startsWith('-#')));
...
check('index.html : 7 références ?v=323', (read('public/index.html').match(/\?v=323/g) || []).length === 7);
check('index.html : plus aucune référence ?v=232', !read('public/index.html').includes('?v=232'));
check('sw.js : cache botdev-v241', read('public/sw.js').includes("const CACHE = 'botdev-v323';"));
