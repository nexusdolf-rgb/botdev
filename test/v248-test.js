...
(async () => {
  ...
  check('index.html : ?v=305 référencé 7 fois',
    versions.length === 7 && versions.every((v) => v === '?v=305'),
    `${versions.length} refs : ${[...new Set(versions)].join(',')}`);
  check('sw.js : cache « botdev-v305 »', swSource.includes("const CACHE = 'botdev-v305';"));
  check('index.html et sw.js portent la même version',
    swSource.includes('botdev-v305') && versions.every((v) => v === '?v=305'));
