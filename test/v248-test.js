...
(async () => {
  ...
  check('index.html : ?v=324 référencé 7 fois',
    versions.length === 7 && versions.every((v) => v === '?v=324'),
    `${versions.length} refs : ${[...new Set(versions)].join(',')}`);
  check('sw.js : cache « botdev-v324 »', swSource.includes("const CACHE = 'botdev-v324';"));
  check('index.html et sw.js portent la même version',
    swSource.includes('botdev-v324') && versions.every((v) => v === '?v=324'));
