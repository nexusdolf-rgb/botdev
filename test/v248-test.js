...
(async () => {
  ...
  check('index.html : ?v=304 référencé 7 fois',
    versions.length === 7 && versions.every((v) => v === '?v=304'),
    `${versions.length} refs : ${[...new Set(versions)].join(',')}`);
  check('sw.js : cache « botdev-v304 »', swSource.includes("const CACHE = 'botdev-v304';"));
  check('index.html et sw.js portent la même version',
    swSource.includes('botdev-v304') && versions.every((v) => v === '?v=304'));
