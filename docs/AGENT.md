...
- Commits en français, préfixés par un numéro de version (dernier : **v331**) avec description détaillée
...
- **v331 (26/09 — TICKETS CLASSIQUES : TOUT DANS LA CARTE)** :
  comme le système avancé : rien en dessous. Textes, types, MP et
  salon du ticket sont *dans* le panneau bouton ; textes + autres
  menus *dans* le panneau menu. IDs/APIs inchangés. test/v331-test.js.
- **v330 (26/09 — TICKETS CLASSIQUES : MOINS DE TEXTE)** :
  page Tickets seulement. Les cartes bouton / menu / extras / types
  sont rangées comme le système avancé : descriptions courtes, textes
  optionnels repliés (`<details>`). IDs `t-send` / `tm-send` / `adv-send`
  et APIs extra-menus inchangés. test/v330-test.js.
- **v329 (26/09 — RÉACTION : TITRE + EMOJI MODIFIABLES)** :
  le titre du message « Rôles par réaction emoji » et son emoji
  se règlent dans le dashboard. test/v329-test.js.
...
- **v325 (25/09 — SÉLECTEURS MOBILE : CLAVIER QUI RESTE OUVERT)** :
  ...
  pour les chiffres. La feuille se lève au-dessus du clavier.
  test/v325-test.js.
...
- **v323 (24/09 — MODULE LIENS)** :
  ...
  embed personnalisable : le lien est écrit dans l’encadré ET en bouton
  cliquable. test/v323-test.js.
...
- **v319 (23/09 — CONNEXION : PHOTO DE PROFIL OPTIMUS, PAS LA TÊTE 🤖 ORANGE)** :
  ...
  orange. Pas la bannière. test/v319-test.js.
- **v318 (23/09 — NETTOYAGE AUTO : DES PLUS ANCIENS VERS LES PLUS RÉCENTS)** :
  dès que c’est activé, ça part tout de suite (pas besoin d’attendre un
  nouveau message). On remonte l’historique et on retire du plus ancien
  vers le plus récent, un message toutes les X secondes. test/v318-test.js.
...
- Dernière version : **v331** — page Tickets : chaque système a
  tout *dans* sa carte (rien en dessous), comme l’avancé.
  `test/v331-test.js`.
- **v330** — page Tickets : moins de texte, blocs repliés.
  `test/v330-test.js`.
- **v329** — rôles par réaction : titre du message
  Discord + emoji du titre modifiables. `test/v329-test.js`.
- **v328** — tickets : on peut ajouter d’autres
